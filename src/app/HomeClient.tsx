...
export default function HomeClient({
  ...
  const handleSendChatMessage = async (e: React.FormEvent) => {
    ...
    const messagePayload = {
      ...
      userName: userName, // Pass username so it gets upserted if it's the first message
    };
  ...
  return (
    <div className={styles.container}>
        ...
        <div className={styles.dialerOverlay}>
          <div className={styles.dialerContent}>
            <div className={styles.dialerHeader}>
              <div style={{ display: 'inline-flex', padding: 12, borderRadius: '50%', background: 'rgba(212, 175, 55, 0.05)', color: 'var(--accent-gold)' }}>
                <Phone size={24} />
              </div>
      ...
      {selectedProject && (
        <div className={styles.projectModalOverlay} onClick={() => setSelectedProject(null)}>
          <div className={styles.projectModalContent} onClick={e => e.stopPropagation()}>
            <div style={{ position: 'relative' }}>
              <button onClick={() => setSelectedProject(null)} className={styles.projectModalClose}>
                <X size={20} />
              </button>
              <img src={selectedProject.image} alt={selectedProject.title} className={styles.projectModalImg} />
            </div>

            <div className={styles.projectModalMeta}>
              <h2 className={`${styles.projectModalTitle} gold-gradient-text`}>{selectedProject.title}</h2>
              <p className={styles.projectModalDesc}>{selectedProject.description}</p>
              
              {selectedProject.richText && (
                <div 
                  className={styles.projectModalRich}
                  dangerouslySetInnerHTML={{ __html: selectedProject.richText }}
                />
              )}
